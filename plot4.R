...
library(data.table)
dataset <- fread("household_power_consumption.txt", select=c("Date","Time","Sub_metering_1","Sub_metering_2","Sub_metering_3","Global_active_power","Global_reactive_power","Voltage"))
...
# topleft
plot(subds$dt,subds$Global_active_power,type="l",xlab="",ylab="Global Active Power",cex.axis=0.7,cex.lab=0.7)

#topright
plot(subds$dt,subds$Voltage,type="l",xlab="datetime",ylab="Voltage",cex.axis=0.7,cex.lab=0.7)
...
legend('topright', c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), inset=-0.04, y.intersp=0.5, bty="n", lty=1, cex=0.7, col=c('black', 'red', 'blue'),xjust=1,yjust=1,text.width = strwidth("Sub_meterin"))

#bottomright
plot(subds$dt,subds$Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power",cex.axis=0.7,cex.lab=0.7)
